...
const SHELL=['./','./index.html','./styles.css','./app.js','./manifest.webmanifest','./offline.html','./icons/icon-192.png','./icons/icon-512.png','./icons/icon-maskable-512.png'];
...
function isShellPath(url){const p=url.pathname.replace(/\/+$/,'/');return /\/(BOARD\/)?(index\.html|styles\.css|app\.js|manifest\.webmanifest|offline\.html)?$/i.test(p)||/\/icons\/(icon-192|icon-512|icon-maskable-512)\.png$/i.test(url.pathname);}
...
self.addEventListener('fetch',event=>{const request=event.request;if(request.method!=='GET')return;const url=new URL(request.url);if(url.origin!==self.location.origin||requestIsPrivate(request,url))return;
  if(request.mode==='navigate'){
    event.respondWith(fetch(request,{cache:'no-store'}).catch(()=>caches.match('./offline.html')));return;
  }
